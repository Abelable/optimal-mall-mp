  ...
  methods: {
    ...
    async likeClick() {
      const anmationData = {
        ...
        image: this.properties.manual
          ? [...imagesArr, userAvatar][
              Math.floor(Math.random() * (imagesArr.length + 1))
            ]
    ...
    bubbleAnimate() {
      Object.keys(queue).forEach(key => {
        ...
        ctx.beginPath();
        ctx.arc(
          x + 15 * anmationData.imageSize,
          y + 15 * anmationData.imageSize,
          15 * anmationData.imageSize,
          0,
          2 * Math.PI,
          false
        );
